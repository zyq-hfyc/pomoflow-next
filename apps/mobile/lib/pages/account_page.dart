...
class AccountPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: _buildAppBar(
        ...
        theme,
        cfg.title,
        // 顶部 QR 图标(用户 2026-09-06 反馈):只在 home 概览页显示,
        // 点进二级模块后该入口退场(用户已在该模块里,不再需要"我的二维码")。
        actions: initialSection == 'home'
            ? [
                Padding(
  ...
  /// home 顶栏 QR 图标 → 个人信息二维码占位页(后续接 qr_flutter)。
  static void _openMyQr(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const _AccountQrPage()),
    );
  }
...
/// 二级「我的二维码」占位页(用户 2026-09-06 反馈):首页顶栏 QR 图标 →
/// 类似微信「我的二维码」,他人扫码加好友。**当前为占位** —— 真实二
/// 维码生成待接入 qr_flutter;用户标识 payload 待定(user_id? phone?)。
/// 占位视觉同 me_page 的推荐页:brand 主色四角方括号 + 中央网格。
