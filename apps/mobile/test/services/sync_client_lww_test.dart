...
bool _localWins(int localRev, int localMs, String localDev,
    int remoteRev, int remoteMs, String remoteDev) {
...
void main() {
  group('LWW resolveConflict - 对照 server crates/core/src/sync/lww.rs:33-54', () {
    test('revision 大者本地胜', () {
    ...
    test('修订并 padded:与 server lww.rs 4 个测试对齐', () {
      ...
      final cases = <_Case>[
        ...
        const _Case(5, 1, 'a', 4, 999, 'z', true),  // rev: local>remote → local
        const _Case(5, 2, 'a', 5, 1, 'a', true),  // rev: 同, ms: local>remote → local
        const _Case(5, 2, 'b', 5, 2, 'a', true),  // rev+ms: 同, dev: b>a → local
...
class _Case {
  const _Case(this.lr, this.lm, this.ld, this.rr, this.rm, this.rd, this.expected);
  final int lr, lm;
  final String ld;
  final int rr, rm;
  final String rd;
