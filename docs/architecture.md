...
- **关键安全语义**:找回密码与换绑邮箱均触发**全端踢出**;换绑邮箱必须验
  ...
  其他设备下线 + 本机换新令牌)。
- **裁剪决策(本期明确不做)**:手机短信/QQ/企业微信渠道(个人无企业资质)、2FA(P4)。
  **提前落地(2026-08-22 用户决策,均按参考原型规格)**:
  ① 个性签名(users.bio,≤50 字,原列 P6);
  ② **头像上传/移除**(原列 P6)—— JPG/PNG ≤2MB,**PG BYTEA 直存**(base64 JSON
  传输,服务端魔法字节核验;个人规模免对象存储,P6 规模化再迁 COS+CDN);
  ③ **数据导出**(原列 P4,随注销)—— GET /v1/auth/export 返回 profile 概要 +
  全部云端快照,客户端保存为 JSON 备份(注销前建议先导出);
  ④ 账号注销(原列 P4)—— 验证码(purpose=delete,已绑邮箱者必填)+ 当前密码 +
  输入"注销账号"确认 + 勾选须知 → `deletion_requested_ms` 标记 + 其他设备下线
  (保留本机作撤销通道)→ **15 天冷静期**内可凭密码自助撤销;每日维护任务对到期
  账号级联删除(refresh_tokens / auth_identities / snapshots / changelog / users),
  本地数据不受影响但账号无法再登录同步。
...
3. **分阶段实施**:
   ...
   - **P6(远期,2026-08-22 决策)**:**社交化** ——(头像与个性签名已于 P1d
     提前落地,头像规模化时迁对象存储)好友/关注、专注房间(多人实时一起专注)、周榜与成就、
